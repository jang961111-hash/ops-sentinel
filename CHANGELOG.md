...
포트폴리오 마무리 정리 릴리스. 오늘 하루의 전체 여정을 한 문단으로 기록한다. P0~P2
마스터플랜 17개 이슈로 기능 구현을 마친 뒤(`v1.0.0`), 독립 architect(Opus) 검증에서
1차 REJECTED 판정(감사로그 무결성·예외처리·MyBatis DB호환성 결함 5건, C1~C5)을 받아
전부 실제로 재현한 뒤 수정하고 재검증을 받아 APPROVED로 뒤집었다(`v1.0.1`). 이어서
사전지식 없는 별도 에이전트가 실제 제출용 zip을 풀고 서버를 띄워 채점하는 콜드스타트
채점 시뮬레이션을 거쳤는데, 이 과정에서 동일 리소스에 40건 동시요청이 몰리면 H2 락
타임아웃이 `JpaSystemException`으로 감싸져 재시도 목록을 벗어나 500 에러로 새는
동시성 결함이 새로 드러났다. 이 결함을 근본원인까지 추적해 재시도 예외 목록을
보강하고 HikariCP 커넥션 풀을 30→60으로 늘려 완전히 해결했으며, 40건 동시요청을
3회 반복 재검증해 매회 500 0건·201 40건·감사로그 40/40 기록(3회 합산 120/120건,
유실 0건)의 무손실 재현을 확인하고 Docker Compose(app+PostgreSQL) 실기동까지
재검증을 마쳤다(`v1.0.2`). 이번 릴리스는 그 위에서 저장소를 포트폴리오로도 손색없게
정리한 것으로, README 최상단에 프로젝트 최종상태 요약을, 하단에 우선순위별 향후
발전 계획을 추가했다.
...
- **(C2) 부하 상황에서 실패 감사로그가 통째로 소실되던 문제** — `AuditLogRecorder.recordFailure()`(REQUIRES_NEW, 별도 DB 커넥션 필요)가 커넥션풀 고갈 등으로 트랜잭션을 여는 단계에서부터 자체 실패하면, 그 예외가 원래 비즈니스 예외를 대체해버려 `catch` 블록의 `throw ex`(원래 예외 재던지기)에 도달하지 못했다. 실측: HikariCP 풀을 3으로 인위적으로 좁힌 뒤 40개 동시요청을 보내자 클라이언트 500 실패가 40건 발생했는데, 이 중 FAIL 감사로그는 단 4건만 남았다(약 90% 유실). `AuditLogAspect`에서 `recordSuccess`/`recordFailure` 호출 자체를 try-catch로 한 번 더 감싸, 감사기록 자체의 실패는 로그로만 남기고 원래 예외/정상 흐름을 절대 덮어쓰지 않도록 수정 — 독립 재검증(architect, 동일 조건 재현) 결과 39건의 실패 중 34건(약 87%)이 FAIL 감사로그로 남는 것을 확인, 수정 전 대비 대폭 개선됨. 단, 감사기록 자체가 DB 커넥션을 얻지 못하는 극단적 상황에서는 그 몇 건까지 완전히 막을 수는 없다(정상 설정값인 pool 30에서는 40/40 전량 기록되어 실서비스 조건에서는 문제되지 않음). (`AuditLogAspect.java`)
